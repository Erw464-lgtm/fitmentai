  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/waitlist", {
        ...
        body: JSON.stringify({
          ...form,
          neededFeature: "Product feedback / contact request",
          note: `Contact request: ${form.note}`,
        }),
      });
      const result = (await response.json()) as { error?: string; message?: string; demoMode?: boolean };

      if (!response.ok) {
        throw new Error(result.error || result.message || "Your message could not be saved right now.");
      }

      setState("success");
      setMessage(result.demoMode ? "Demo mode: form works, but database storage is not connected." : "Saved. Thanks for helping shape FitmentAI.");
      setForm({ name: "", email: "", role: "Car enthusiast", note: "" });
    } catch (error) {
      setState("error");
      setMessage(error instanceof Error ? error.message : "Your message could not be saved right now.");
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4">
      <div className="grid gap-4 md:grid-cols-2">
        <Input label="Name" value={form.name} onChange={(value) => setForm((current) => ({ ...current, name: value }))} required />
      ...
      <label className="grid gap-2 text-sm font-medium text-[#b8ac91]">
        What should FitmentAI help with?
        <textarea
          value={form.note}
          ...
          placeholder="Example: I want to compare spoiler fitment for my G80 M3, save builds, and see verified install examples."
        />
      </label>
      <button
        type="submit"
      ...
      >
        {state === "loading" ? <Loader2 className="h-4 w-4 animate-spin" /> : <MessageSquareText className="h-4 w-4" />}
        Send Feedback
      </button>
      {message ? (
        <p className={`rounded-lg border p-3 text-sm ${state === "error" ? "border-warning/35 bg-warning/10 text-orange-200" : "border-signal/30 bg-signal/10 text-[#bfe5c6]"}`}>
          {message}
        </p>
      ) : null}
    </form>
